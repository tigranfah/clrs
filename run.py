...
def canonicalize_feedback(feedback, task_name: str):
    """
    Normalize features into a canonical graph-algorithm format.
    Hints and outputs are passed through unchanged.
    """

    inputs = {dp.name: dp for dp in feedback.features.inputs}

    # --- required canonical inputs ---
    canonical_inputs = []

    # adj (edge mask) – required
    canonical_inputs.append(inputs["adj"])

    # A (edge weights) – if missing, derive from adj
    if "A" in inputs:
        canonical_inputs.append(inputs["A"])
    else:
        adj = inputs["adj"]
        A = clrs.DataPoint(
            name="A",
            location=adj.location,
            type=clrs.Type.SCALAR,
            data=adj.data.astype(np.float32),
        )
        canonical_inputs.append(A)

    # pos (node scalar)
    if "pos" in inputs:
        canonical_inputs.append(inputs["pos"])

    # s (source node)
    canonical_inputs.append(inputs["s"])

    # task_id (graph-level scalar)
    task_id = clrs.DataPoint(
        name="task_id",
        location=clrs.Location.GRAPH,
        type=clrs.Type.SCALAR,
        data=np.full(
            (feedback.features.lengths.shape[0],),
            ["bfs_", "dfs_", "dijkstra_"].index(task_name),
            dtype=np.float32,
        ),
    )
    canonical_inputs.append(task_id)

    new_features = clrs.Features(
